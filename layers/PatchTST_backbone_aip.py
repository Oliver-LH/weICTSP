...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        ###################### AIP ###################### 
        self.conv_size = [15, 49, 145, 385]
        padding_max_len = 21
        self.seq_len = 720
        self.ema_alpha = nn.Parameter(0.95*torch.ones(padding_max_len))
        self.std_weights = nn.Parameter(torch.cat([1*torch.ones(len(self.conv_size), padding_max_len), 
                                                      torch.ones(1, padding_max_len)]))
        self.channel_w = nn.Parameter(torch.ones(padding_max_len), requires_grad=True)
        self.channel_b = nn.Parameter(torch.zeros(padding_max_len), requires_grad=True)
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        if self.revin: 
            ...
            ###################### AIP ###################### 
            eps = 1e-5
            anchor_context = ema_3d(z[:, 0:(self.seq_len), :], alpha=self.ema_alpha[0:z.shape[-1]].clip(0.000001, 0.999999))
            std = weighted_std(z[:, 0:(self.seq_len), :], self.conv_size+[self.seq_len], gate_activation(self.std_weights*2).unsqueeze(0).repeat(z.shape[0], 1, 1)[:, :, 0:z.shape[-1]]).clip(1e-4, 5000)
            context_shape = z.shape
            channel_w = self.channel_w[0:z.shape[-1]]
            channel_b = self.channel_b[0:z.shape[-1]]
            z = ((z - anchor_context)/(std+eps)) * self.channel_w + self.channel_b
            ###################### AIP ###################### 
            z = z.permute(0,2,1)
        ...
        if self.revin: 
            ...
            ###################### AIP ###################### 
            z = ((z - channel_b)/channel_w) *(std+eps) + anchor_context
            ###################### AIP ###################### 
            z = z.permute(0,2,1)
        return z
