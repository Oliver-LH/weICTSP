...
class Dataset_ETT_hour(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if bsfffc != 0:
            mean = self.data_x[s_begin+bsfffc:s_end].mean(axis=-1, keepdims=True)
            std = self.data_x[s_begin+bsfffc:s_end].std(axis=-1, keepdims=True) + 1e-8
            masking = np.ones_like(self.data_x[s_begin:s_begin+bsfffc])
            masking = masking * mean / std
        else:
            masking = np.zeros_like(self.data_x[s_begin:s_begin+bsfffc])
        seq_x = np.concatenate((masking, self.data_x[s_begin+bsfffc:s_end]), axis=0)
        seq_y = self.data_y[r_begin:r_end]
